...
struct SolutionRandomKDTree {
    ...
    static void buildGroups(Range range,
        ...
        if (rangeSize < maxGroupSize) {
            ...
            tbb::parallel_invoke(
                [&]{ buildGroups(subRange1, knnIterations, maxGroupSize, points, indices, completed); },
                [&]{ buildGroups(subRange2, knnIterations, maxGroupSize, points, indices, completed); }
            );
        }
    }
