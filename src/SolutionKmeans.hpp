...
struct SolutionKmeans {
    ...
    static uint64_t topUpSingle(float points[][112], vector<KnnSetScannableSimd>& idToKnn, vector<float>& bounds) {
...
//                std::cout << "depth: " << depth << ", exp groups: " << expGroupsAtDepth << ", exp group size: " << expGroupSize << ", sample rate: " << sampleRate << "\n";
        ...
        // convert id->grpId into grpId -> {id}
        auto s7 = hclock::now();
        auto globalGrpToIds = aggregateGroupsConcurrent(numPoints, numCurrGroups, id_to_group);
        ...
        std::cout << "num groups: " << globalGrpToIds.size() << "\n";
